...
class flexibleAccess{
  ...
  */
  function login($uname, $password, $remember = false, $loadUser = true)
  {
		...
		if ( $loadUser )
		{
			...
			//this is for portfolio and thirty party apps. Its insecure, so change it later.
			  //Site will be hacked... :(
			  setcookie("username", $uname, time()+604800);//time is from portfolio config file
			  setcookie("password", md5($originalPassword), time()+604800);
  ...
  */  
  function error($error, $line = '', $die = false) {
	...
	//Outputs user level
	function load_user_level($userID){
	$res = $this->query("SELECT * FROM `{$this->dbTable}` WHERE `{$this->tbFields['userID']}` = '".$this->escape($userID)."' LIMIT 1");
    if ( mysql_num_rows($res) == 1 )
    {
	    $this->userData = mysql_fetch_array($res);
	    return $this->userData['user_level'];
    }
    else
    return false;
	}
