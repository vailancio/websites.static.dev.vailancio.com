...
$(function(){
			...
			//$('#home-href').removeClass("active");
			//$('#aboutus-href').addClass("active");
			var currentdiv = $('#aboutus-page');
		
			var a = $(currentdiv).position().left;
...
//inview
   ...
   var bodyWidth = 0;
	...
	$(".table tr td").each(function(){
        	bodyWidth = parseInt(parseFloat($(this).width())) + parseInt(bodyWidth);
      });
if (delta > 0) {
	left -=70;
	var docpos = window.pageXOffset;
	
change_tab();
	if(docpos>='0'){
	
     //$('html,body').scrollLeft=left+'px';
 $('html,body').stop().animate({
 ...
 $('html,body').stop().animate({
	...
	$(".table tr td").each(function(){
        ...
        //alert(' width: '+bodyWidth+' scroll: '+scrollLeft+' % '+percent)
        if(percent <= 13.74){
        ...
        if(percent >= 13.74 && percent <= 29.24){
